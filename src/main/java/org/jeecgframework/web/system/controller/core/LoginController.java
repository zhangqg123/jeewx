...
public class LoginController extends BaseController{
	...
	public String login(ModelMap modelMap,HttpServletRequest request) {
		...
		if (user != null) {
			...
			List<Map> qywxAgents=new ArrayList<Map>();
			for (QywxAgent  qywxagent:agentList) {
			...
			String qywxAccountId = agentList.get(0).getAccountId();
			request.getSession().setAttribute(WeiXinConstants.QYWX_ACCOUNT_ID, qywxAccountId);
			request.getSession().setAttribute(WeiXinConstants.QYWX_AGENTS, qywxAgents);
			...
			request.getSession().setAttribute("departId", user.getTSDepart().getId());
			request.getSession().setAttribute("roles", roles);
        	System.out.println("....departId..."+user.getTSDepart().getId());
			// 默认风格
	...
	public String login2(ModelMap modelMap,HttpServletRequest request, HttpServletResponse response) throws IOException {
		DataSourceContextHolder.setDataSourceType(DataSourceType.dataSource_jeecg);
		TSUser user = ResourceUtil.getSessionUserName();
		String roles = "";
		if (user != null) {
			List<TSRoleUser> rUsers = systemService.findByProperty(TSRoleUser.class, "TSUser.id", user.getId());
//			TSRole role = rUsers.get(0).getTSRole();
//			String roleCode = role.getRoleCode();
			...
			request.getSession().setAttribute("departId", user.getTSDepart().getId());
			request.getSession().setAttribute("roles", roles);
        	System.out.println("....userName..."+user.getUserName()+"....departId...."+user.getTSDepart().getId());
//        	if(roleCode!=null&&roleCode!=""&!roleCode.equals("admin")){
           	if(roles!=null&&roles!=""){
//				response.sendRedirect("p3/"+roleCode+"/index.do?articleList");        		
				response.sendRedirect("p3/xzkx/index.do?articleList");        		
        	}
//        	if(user.getTSDepart().getId().equals("340")){
//				response.sendRedirect("p3/notice/index.do?articleList");
//        	}
			return null;
		} else {
			return "login/login2";
