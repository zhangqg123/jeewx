...
public class AuthInterceptor implements HandlerInterceptor {
	...
	private List<String> qyUsersContainUrls;
	/**
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object object) throws Exception {
		...
		if (excludeUrls.contains(requestPath)) {
			...
			if (client != null && client.getUser()!=null ) {
				...
				Map<String, String> userinfo=(Map<String, String>) session.getAttribute(WeiXinConstants.QY_USERINFO);
				if(userinfo!=null&&userinfo.get(WeiXinConstants.QY_AUTH).equals("auth")){
					if(moHuContain(qyUsersContainUrls, requestPath)){
						return true;
					}else{
						return false;
					}
				}else{
					if(requestPath.contains("index.do")){
						String baseurl = ResourceUtil.getConfigByName("domain");
						response.sendRedirect(baseurl+"/loginController.do?login2");
					}
					return false;
				}
			}
	...
	public List<String> getQyUsersContainUrls() {
		return qyUsersContainUrls;
	}

	public void setQyUsersContainUrls(List<String> qyUsersContainUrls) {
		this.qyUsersContainUrls = qyUsersContainUrls;
	}
}
