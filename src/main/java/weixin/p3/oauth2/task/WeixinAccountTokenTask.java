...
import org.oms.qiye.util.WechatAccessToken;
...
//		long qystart = System.currentTimeMillis();
//		if(list.size()>0){
...
//	
//			try {
//				resetQywxToken(qywxAccount);
//			} catch (Exception e) {
//				LogUtil.info("---------定时任务【重置超过2小时失效token】失败公众号------------------"+qywxAccount.getAccontName());
//			}
//		}	
//		
//		long qyend = System.currentTimeMillis();
//		long qytimes = qyend - qystart;
//		org.jeecgframework.core.util.LogUtil.info("====================定时任务【重置超过2小时失效token】结束，" + "====执行重置企业号数量："+ (list!=null?list.size():0) +" ========总耗时"+qytimes+"毫秒==========");
	}
	
	private void resetQywxToken(QywxAccount qywxAccount){
		String token = null;
		// 失效 重新获取
		String requestUrl = WechatAccessToken.company_access_token_url.replace("CORPID", qywxAccount.getCorpid()).replace("CORPSECRET", qywxAccount.getSecret());
		...
		if (null != jsonObject) {
			try {
				...
				LogUtil.info("---------定时任务定时任务【重置超过2小时失效token】成功企业号------------------" + qywxAccount.getAccontName());
			} catch (Exception e) {
				// 获取token失败
				...
				LogUtil.info("---------定时任务定时任务【重置超过2小时失效token】失败保存公众号------------------" + qywxAccount.getAccontName());
			}
		}
	}
