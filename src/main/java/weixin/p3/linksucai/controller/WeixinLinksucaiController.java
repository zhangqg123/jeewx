...
//			if(t.getWxtype().intValue()==2){
//				t.setAccountid(ResourceUtil.getQywxAccountId());
//			}
		}
	...
	public void link(WeixinLinksucaiEntity weixinLinksucai,HttpServletRequest request, HttpServletResponse response) {
		...
		String outUrl = null;
		CmsArticleEntity cmsArticleLinksucai;
		...
		//微信公众账号ID
		accountid = weixinLinksucai.getAccountid();
	    ...
	    if(oConvertUtils.isNotEmpty(jwid)){
			...
			outUrl = weixinLinksucai.getOuterLink();
			String agentId = outUrl.split("[=]")[3];
//			String baseurl = ResourceUtil.getConfigByName("domain");
//			outUrl=baseurl+"/cmsController.do?"+aa;
		...
		if(oConvertUtils.isEmpty(outer_link_deal)){
			if(oConvertUtils.isNotEmpty(jwid)){
				...
				qyUserId = ResourceUtil.getQyUserId();
				System.out.println("------------------begin----------begin1-------------------");
				outer_link_deal = outUrl+"&qyUserId="+qyUserId;
				System.out.println("------------------begin----------begin2-------------------");
			}
